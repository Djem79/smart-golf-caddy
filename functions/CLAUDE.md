...
- `deleteAccount` — удаление аккаунта (требование App Store 5.1.1(v)).
  ...
  **Инвариант: чужие данные не трогаем.** Соло-раунды удаляются целиком,
  групповые — только обезличиваются (`players[uid]`: имя → «Удалённый
  игрок», avatar и email стираются); `playerIds`, удары и счёт остаются,
  иначе поедут таблица и match-результат товарищей.
