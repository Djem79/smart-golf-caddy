...
5. **`WatchRoundViewModel.shotsByHole` (in-memory) рехидратируется из
   `WatchShotQueue` (durable) при первом посеве лунки, а не только из
   `snapshot.myShots`.** watchOS штатно выгружает приложение из памяти —
   без рехидратации реальный, ещё не подтверждённый удар "терялся из
   вида" при пересоздании VM (durable confirmedCount обгонял локальный
   счёт), и следующий тап на "+" считал хвост пустым, а
   `enqueue(clubs: [])` стирал durable-запись вместо постановки нового
   удара. `seedIfNeeded` сеет префикс `max(hole.myShots, durable
   confirmedCount)` заглушками ПЛЮС реальный хвост из `shotQueue.pending`
   для этого слота, если он там ещё есть. `syncQueue(allowClear:)`
   дополнительно гарантирует, что `addShot()` НИКОГДА не опустошит
   существующий durable-хвост как побочный эффект рассинхрона — снять
   слот целиком может только настоящий `removeShot()`.
