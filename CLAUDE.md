...
Интерфейс двуязычный. Механизм СВОЙ, без библиотек, и он одинаков по духу
на всех платформах: два словаря плюс типизация, при которой **пропущенный
перевод — ошибка компиляции, а не пустое место на экране**.

- Веб: `src/i18n/` — `ru.ts` канонический, `en.ts` объявлен как
  `Dictionary = typeof ru`. Плюрализация — `plural(n, locale, forms)` на
  `Intl.PluralRules` (в браузере он уже есть, react-i18next добавил бы
  ~40 КБ ради двух языков). Прежний `pluralRu` и `src/utils/intl.ts`
  удалены — двух механизмов быть не должно.
- iOS и часы: `ios/SmartGolfCaddy/Models/Localization/` (`Strings.swift`,
  `PluralForms.swift`, `AppLocaleStore.swift`), каталог подключён к
  watch-таргету ссылкой. **String Catalogs (`.xcstrings`) отвергнуты
  доказанно**: `String(localized:locale:)` игнорирует явный аргумент
  `locale:` и всегда берёт язык устройства, поэтому переключение внутри
  приложения с ними невозможно. Не возвращать.
- Письма: `functions/src/i18n/` — отдельные словари (functions это
  независимый проект, зависимости на `src/` быть не должно).

Язык: по умолчанию системный, выбор пользователя перекрывает и хранится в
`AppUser.locale` (`users/{uid}`), поэтому синхронизируется между
устройствами и доступен серверу. Часы своего экрана настроек не имеют —
язык приезжает в снимке раунда (`WatchRoundSnapshot.locale`).

**Письма рендерятся на языке ПОЛУЧАТЕЛЯ**, а не отправителя: в групповом
раунде игроки с разными языками получают разные письма (цикл по
`playerIds`, локаль резолвится на каждой итерации). Нет профиля или поля —
русский, как было раньше.

Что НЕ переводится намеренно: `Clubs.penaltyId = "Штраф"` и `PENALTY_ID`
(значение данных, ключ синхронизирован между платформами и базой —
переводится только подпись через `getClubLabel`), подписи языков
«Русский»/«English» (чтобы их узнал тот, кто случайно включил чужой язык),
и гольф-нотация match play (`AS`, `2 UP`, `3&2`) — она международная.

Известный долг: строка «Удалённый игрок» пишется сервером в базу
по-русски. В письмах она подменяется переводом при сборке, а в интерфейсе
веба и iOS показывается как есть.
