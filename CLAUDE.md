...
5. **`WatchRoundViewModel.shotsByHole` (in-memory) рехидратируется из
   `WatchShotQueue` (durable) при первом посеве лунки по ОБЩЕМУ РАЗМЕРУ
   лунки, а не сложением слагаемых вслепую.** watchOS штатно выгружает
   приложение из памяти — без рехидратации реальный, ещё не
   подтверждённый удар "терялся из вида" при пересоздании VM (durable
   confirmedCount обгонял локальный счёт), и следующий тап на "+" считал
   хвост пустым, а `enqueue(clubs: [])` стирал durable-запись вместо
   постановки нового удара. `confirmedCount` и `pendingClubs.count`
   дизъюнктны МЕЖДУ СОБОЙ (markConfirmed срезает подтверждённый префикс
   из pending), но `hole.myShots` (снимок, едет через
   `updateApplicationContext`) и `confirmedCount + pendingClubs.count`
   (квитанция, едет через `transferUserInfo`) — два НЕЗАВИСИМЫХ канала,
   которые могут описывать ОДНО И ТО ЖЕ событие, обогнав друг друга в
   любую сторону: наивное `max(myShots, confirmedCount)` заглушками
   ПЛЮС `pending.clubs` задваивало удар, если снимок уже обогнал
   квитанцию. Верная формула — `total = max(hole.myShots, confirmedCount
   + pendingClubs.count)`, число заглушек — `total - pendingClubs.count`,
   хвост — сам `pendingClubs`. `syncQueue(allowClear:)` дополнительно
   гарантирует, что `addShot()` НИКОГДА не опустошит существующий
   durable-хвост как побочный эффект рассинхрона — снять слот целиком
   может только настоящий `removeShot()`.
6. **`WatchShotQueue.installId` (durable UUID установки часов) защищает
