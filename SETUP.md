...
- На часах включён **Developer Mode** (Настройки → Конфиденциальность и
  ...
  1. Подключить iPhone к Маку кабелем, разблокировать, «Доверять этому
     компьютеру».
  2. На iPhone включить Режим разработчика (появится после шага 1; если
     нет — открыть `Window → Devices and Simulators` в Xcode при
     подключённом телефоне), перезагрузить телефон и подтвердить.
  ...
  4. Теперь на часах появится тот же пункт — включить, часы
     перезагрузятся.
  Проверить, что Mac вообще видит устройства: `xcrun xctrace list devices`
  — телефон не должен быть в разделе «Devices Offline». после первой установки часы попросят перейти в
