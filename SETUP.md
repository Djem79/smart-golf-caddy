...
- На часах включён **Developer Mode** (Настройки → Конфиденциальность и
  ...
  1. Поднять соединение Mac ↔ iPhone. Кабель НЕ обязателен: если телефон
     уже сопряжён (`xcrun devicectl list devices` показывает
     `available (paired)`), достаточно разблокировать его и держать в
     одной сети Wi-Fi с Маком. Кабель нужен только для ПЕРВОГО сопряжения
     либо когда беспроводное соединение не поднимается
     (`tunnelState: unavailable` держится, телефон в Xcode серый).
