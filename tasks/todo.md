...
| 2 | Sign in with Apple | ГОТОВО 02.09.2026: App ID + capability, Services ID, ключ, провайдер в Firebase (OAuth code flow), веб задеплоен, iOS-сборка на устройстве — вход через Apple подтверждён владельцем («всё сработало»). Кнопка iOS: фикс высоты 64e36ca. Остаток §5 (удаление с отзывом, «Скрыть e-mail») — при обкатке в TestFlight |
...
Веб-кнопка Apple спрятана за `VITE_APPLE_SIGNIN_ENABLED=true` (в прод
ушла сборка БЕЗ кнопки — до включения провайдера она падала бы с
operation-not-allowed); включить флаг при активации.
...
- [ ] Монетизация решена (2026-08-30): TestFlight-обкатка → ПЛАТНЫЙ
      ...
      (договора ОАЭ–США нет). После оплаты аккаунта сразу запустить
      Paid Applications Agreement (проверка небыстрая); выбрать цену и
      страны (ЕС = статус Trader с публичными контактами; в РФ оплата
      фактически только со счёта оператора). Детали — в
      docs/appstore/submission-package.md, раздел «Стратегия».
