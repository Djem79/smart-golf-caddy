...
- [ ] Монетизация решена (2026-08-30): TestFlight-обкатка → ПЛАТНЫЙ
      релиз. После оплаты аккаунта сразу запустить Paid Applications
      Agreement (банк + налоговые формы, проверка небыстрая); выбрать
      цену и страны (ЕС = статус Trader с публичными контактами; в РФ
      оплата фактически только со счёта оператора). Детали — в
