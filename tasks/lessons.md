...
- **Смена языка UI ломает e2e-локаторы молча: смоук завязан на видимый
  текст, а язык теперь берётся из navigator.language.** Локализация от
  29.08 уронила playwright-смоук в CI (Chromium по умолчанию en-US, спека
  ищет «Войти через Google») — e2e не гонялись после неё локально, падение
  увидели только по письму от GitHub. Правила: (1) в playwright.config
  локаль прибита (`locale: 'ru-RU'`) — тест не должен зависеть от машины;
  (2) любая правка, меняющая user-facing строки/язык, требует локального
  `npm run test:e2e` до пуша; (3) `npm run test:run` e2e НЕ покрывает —
  это разные раннеры.
- **Системные SwiftUI-контролы (SignInWithAppleButton и т.п.) — задавать
  `frame(height:)`, а не `minHeight`.** Контрол жадный по вертикали и с
  minHeight съедает всё место под Spacer: на симуляторе/в юнит-тестах
  этого не видно, на устройстве вышел чёрный квадрат на полэкрана
